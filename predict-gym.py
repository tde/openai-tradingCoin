...
numTrain = 1
fileModel = os.path.dirname(__file__) + "/trained_models/ppo/model_" + str(numTrain)
model = PPO2.load(fileModel)

obs = env.reset()
env.set_first_index()

result_all = pd.DataFrame(columns=['order_count', 'usd_change', 'coins_change', 'profit'])
for i in range(1000):
...
result_all = pd.concat([row_result, result_all], axis=0)
