...
class Position:
  ...
  def calcProfit(self, closePrice = None):
    ...
    closePriceForce = closePrice if closePrice != None else self.openOrder.price
    ...
    if (self.isClosed == False):
      ...
      dummyOrder = Order(type = type, price = closePriceForce, qty = self.remainQtyForClose)
  ...
  def show(self, index = None): 
    ...
    print (header)
    print ('isClosed: {}, remainQtyForClose: {}, profit: {}'.format(self.isClosed, self.remainQtyForClose, self.calcProfit()))
...
class PositionStore:
  ...
  # сколько осталось монет для закрытия всех позиций
  def calcRemainClose(self):
    remainUsd = remainCoins = 0

    for p in self.positions:
      if (p.isClosed == False):
        if (p.openOrder.type == ActionType.BUY):
          remainCoins += p.remainQtyForClose
        else:
          remainUsd += p.remainQtyForClose

    return remainCoins, remainUsd
